...
class EDSR(object):
    ...
    def __init__(
        ...
        # depth
        x = slim.conv2d(image_input, feature_size, [3, 3])                # conv  # input ( 32*32 ) output ( 32*32 )
        ...
        # layer      *******************
        x = slim.conv2d(x, feature_size, [3, 3])       # conv_65
        x += conv_1      # 补齐残差
    ...
    def predict(self, x):
        print("Predicting...")
        if (len(x.shape) == 3) and not(
                x.shape[0] == self.img_size and x.shape[1] == self.img_size):
            num_across = x.shape[0] // self.img_size
            num_down = x.shape[1] // self.img_size
            tmp_image = np.zeros(
                [x.shape[0] * self.scale, x.shape[1] * self.scale, 3])
            for i in range(num_across):
                for j in range(num_down):
                    tmp = self.sess.run(self.out, feed_dict={self.input: [
                                        x[i * self.img_size:(i + 1) * self.img_size, j * self.img_size:(j + 1) * self.img_size]]})[0]
                    tmp_image[i *
                              tmp.shape[0]:(i +
                                            1) *
                              tmp.shape[0], j *
            ...
            if (x.shape[0] %
                self.img_size != 0 and x.shape[1] %
                    self.img_size != 0):
                tmp = self.sess.run(self.out, feed_dict={self.input: [
                                    x[-1 * self.img_size:, -1 * self.img_size:]]})[0]
                tmp_image[-1 * tmp.shape[0]:, -1 * tmp.shape[1]:] = tmp

            if x.shape[0] % self.img_size != 0:
                for j in range(num_down):
                    tmp = self.sess.run(self.out, feed_dict={self.input: [
                                        x[-1 * self.img_size:, j * self.img_size:(j + 1) * self.img_size]]})[0]
                    tmp_image[-1 * tmp.shape[0]:,
                              j * tmp.shape[1]:(j + 1) * tmp.shape[1]] = tmp
            if x.shape[1] % self.img_size != 0:
                for j in range(num_across):
                    tmp = self.sess.run(self.out, feed_dict={self.input: [
                                        x[j * self.img_size:(j + 1) * self.img_size, -1 * self.img_size:]]})[0]
    ...
    def set_data_fn(self, fn, args, test_set_fn=None, test_set_args=None):
        self.data = fn                    # get_batch fn
        self.args = args                  # args
        self.test_data = test_set_fn      # get_test_set fn
        self.test_args = test_set_args    # args
    ...
    def train(self, iterations=1000, save_dir="saved_models"):
        ...
        with self.sess as sess:
            ...
            train_writer = tf.summary.FileWriter(
                save_dir + "/train", sess.graph)

            # If we're using a test set, include another summary writer for
            # that
            if test_exists:
                test_writer = tf.summary.FileWriter(
                    save_dir + "/test", sess.graph)
                test_x, test_y = self.test_data(*self.test_args)
                test_feed = {self.input: test_x, self.target: test_y}

            # This is our training loop
            for i in tqdm(range(iterations)):
                # Use the data function we were passed to get a batch every
                # iteration
                ...
                # If we're testing, don't train on test set. But do calculate
                # summary
                if test_exists:
